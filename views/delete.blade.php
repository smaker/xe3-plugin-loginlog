<form action="{{ route('settings.loginlog.destroy') }}" method="post">
    {{ csrf_field() }}
    {{ method_field('delete') }}

    <div class="xe-modal-header">
        <button type="button" class="btn-close" data-dismiss="xe-modal" aria-label="Close"><i class="xi-close"></i></button>
        <strong class="xe-modal-title">{{ xe_trans('loginlog::loginlog') }} {{ xe_trans('xe::delete') }}</strong>
    </div>
    <div class="xe-modal-body">

        {{ xe_trans('loginlog::confirmDeleteLogs') }}
        ...
        <ul>
            @foreach($logs as $log)
                <li>
                    ...
                    {{ $log->user->getDisplayName() }}({{ $log->email }})
